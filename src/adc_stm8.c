...
Argument může být buď makro: ADC2_CHANNEL_x ... kde x může být 0 až 15 nebo
číselná hodnota 0 až 15 (odpovídá číslu kanálu)
*/
uint16_t ADC_get(ADC2_Channel_TypeDef ADC2_Channel) {
    ADC2_Select_Channel(
        ADC2_Channel); // vybere kanál / nastavuje analogový multiplexer
    ADC2->CR1 |= ADC2_CR1_ADON; // Start Conversion (ADON must be SET before =>
                                // ADC must be enabled !)
    while (!(ADC2->CSR & ADC2_CSR_EOC))
        ; // čeká na dokončení převodu (End Of Conversion)
...
/* Funkce počká definovaný čas na stabilizaci ADC (vhodné zařadit po spuštění
 * ADC k zajištění že se první převod neprovede před stabilizací)
 */
void ADC2_Startup_Wait(void) { _delay_us(ADC_TSTAB); }
...
uint16_t ADC_get(ADC1_Channel_TypeDef ADC1_Channel) {
    ...
    ADC1->CR1 |= ADC1_CR1_ADON; // Start Conversion (ADON must be SET before =>
                                // ADC must be enabled !)
...
void ADC1_Startup_Wait(void) { _delay_us(ADC_TSTAB); }
